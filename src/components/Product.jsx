...
export default function Product({ producto }) {
  ...
  return (
    <div className="flex flex-row items-center p-3 bg-white rounded-md shadow-md h-36 w-[590px] shadow-black">
      <img src={producto.rutaImg} alt="" className="w-32 h-32 rounded-md" />
      <div>
        <p>{"Producto:" + producto.nombre}</p>
        <p>{"Precio:$" + producto.precio}</p>
        <button
          className="text-[0.9rem] text-white bg-red-500 p-1 mt-2 rounded"
          onClick={deleted}
        >
          Eliminar del stock
        </button>
